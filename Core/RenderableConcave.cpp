...
RenderableConcave::RenderableConcave(const Loop3D& polygon, const QColor& color) : Renderable(GL_TRIANGLES) {
	tessellate(polygon, color);
}

RenderableConcave::RenderableConcave(const Loop3D& polygon, Texture* texture) : Renderable(GL_TRIANGLES) {
...
void RenderableConcave::tessellate(const Loop3D& polygon) {
...
void RenderableConcave::tessellate(const Loop3D& polygon, const QColor& color) {
	if (polygon.size() < 3) return;
