...
WaterRenderer::WaterRenderer(int width, int depth, float elevation) {
	this->width = width;
	this->depth = depth;
...
float WaterRenderer::z(const float x, const float y, const float t) {
	const float x2 = x - 3;
	const float y2 = y + 1;
	...
	return tmpZ * 0.2f + elevation; //tmpZ - 0.0f;
}

/*
** Function called to update rendering
*/
void WaterRenderer::renderMe(TextureManager* textureManager) {
	if (texture == NULL) {
		texture = textureManager->get("data/textures/water.jpg");
	}
	...
	//const float waterWidth = 4000.0f;
