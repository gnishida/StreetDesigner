...
void MyRenderer::render(std::vector<ucore::Renderable*> renderables) {
	for (int i = 0; i < renderables.size(); i++) {
		renderOne(renderables[i]);
	}
}

void MyRenderer::renderOne(ucore::Renderable* renderable) {
 ...
 */
void MyRenderer::renderArea(const Polygon2D& area, GLenum lineType, const QColor& color, float height) {
	std::vector<ucore::Renderable*> renderables;
	renderables.push_back(new ucore::Renderable(lineType, 3.0f));
	renderables.push_back(new ucore::Renderable(GL_POINTS, 10.0f));
...
void MyRenderer::renderPoint(const QVector2D& pt, const QColor& color, float height) {
	ucore::Renderable* renderable = new ucore::Renderable(GL_POINTS, 10.0f);
...
void MyRenderer::renderPolyline(const Polyline3D& polyline, const QColor& color, GLenum lineType) {
	std::vector<ucore::Renderable*> renderables;
	renderables.push_back(new ucore::Renderable(lineType, 3.0f));
	renderables.push_back(new ucore::Renderable(GL_POINTS, 10.0f));
