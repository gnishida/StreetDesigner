...
MyTerrain::MyTerrain(int width, int depth, int cellLength) {
	this->width = width;
	this->depth = depth;
...
MyTerrainCell& MyTerrain::getCell(int idxX, int idxY) {
	if (idxX < 0) idxX = 0;
...
void MyTerrain::init() {
	...
	for (int i = 0; i < numRows; ++i) {
		for (int j = 0; j < numCols; ++j) {
			MyTerrainCell cell = MyTerrainCell(this, j, i, j * cellLength - width/2.0f, i * cellLength - depth/2.0f, cellLength);
			this->cells.push_back(cell);
		}
	}
}

float MyTerrain::getValue(int x, int y) {
 ...
 */
void MyTerrain::setValue(int x, int y, float elevation) {
	int idxX = ((float)x + width/2.0f) / cellLength;
	int idxY = ((float)y + depth/2.0f) / cellLength;
...
void MyTerrain::addValue(int x, int y, float change) {
	int idxX = ((float)x + width/2.0f) / cellLength;
	int idxY = ((float)y + depth/2.0f) / cellLength;
...
/*void MyTerrain::recalcCornersElevation() {
	for (int i = 0; i < numRows; ++i) {
		for (int j = 0; j < numCols; ++j) {
...
float MyTerrain::getSlope(int x, int y) {
	int idxX = ((float)x + width/2.0f) / cellLength;
	int idxY = ((float)y + depth/2.0f) / cellLength;
...
float MyTerrain::getSlopeAtCell(int idxX, int idxY) {
	return getCell(idxX, idxY).getSlope();
}

void MyTerrain::_generateMeshVertices(ucore::TextureManager* textureManager) {
	ucore::RenderableQuadList* renderable = new ucore::RenderableQuadList();
	//ucore::RenderableTriangleList* renderable = new ucore::RenderableTriangleList();
